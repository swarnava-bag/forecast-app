export default function WorkflowGuidePage() {
  return (
    <div className="max-w-5xl space-y-10">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-atlas-ink">Platform Workflow Guide</h2>
      ...
      <Section title="1. User Roles & Permissions">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-atlas-line">
                <th className="text-left py-2 pr-6 text-atlas-ink-muted font-medium">Role</th>
                <th className="text-left py-2 pr-6 text-atlas-ink-muted font-medium">Scope</th>
                <th className="text-left py-2 text-atlas-ink-muted font-medium">Key Capabilities</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-atlas-line/50">
              <RoleRow
                badge="Admin"
                color="amber"
                scope="Full system"
        ...
        <InfoBox>
          New sign-ups default to <strong className="text-atlas-ink">Viewer</strong>. Assign the correct role under <strong className="text-atlas-ink">Admin → Manage Users</strong> before the user can contribute to a cycle.
        </InfoBox>
      </Section>
      ...
      <Section title="3. Forecast Cycle Lifecycle">
        <p className="text-sm text-atlas-ink-muted mb-6">
          Every forecast round is tracked through a <strong className="text-atlas-ink">Cycle</strong> with a month, version number, and status.
          ...
          {[
            ...
            <div key={status} className="flex gap-4">
              <div className="flex flex-col items-center">
                ...
                {i < 6 && <div className="w-px flex-1 bg-atlas-line my-1" />}
              </div>
              <div className="pb-4 flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${statusColors[color]}`}>{status}</span>
                  <span className="text-xs text-atlas-ink-faint">{who}</span>
                </div>
                <p className="text-sm text-atlas-ink">{what}</p>
                <p className="text-xs text-atlas-ink-faint mt-1">{where}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Section title="4. Upload Workflow">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Standard Upload */}
          <div className="space-y-3">
            <p className="text-sm font-semibold text-atlas-ink">Standard (Single Products)</p>
            <Steps steps={[
          ...
          {/* Combo Upload */}
          <div className="space-y-3">
            <p className="text-sm font-semibold text-atlas-ink">Combo Products Upload</p>
            <Steps steps={[
              "Go to Upload page and select an open cycle.",
        ...
        <InfoBox>
          <strong className="text-atlas-ink">Validation rules:</strong> A row is only saved if the SKU is in the SKU Master, the channel is active, and the SKU-Channel mapping exists. Fix failures in Admin → SKU-Channel Map before re-uploading.
        </InfoBox>
      </Section>
      ...
      <Section title="6. Dashboard Features">
        <div className="space-y-4">
          {[
            ...
            <div key={title} className="flex gap-4 p-4 rounded-xl bg-atlas-surface border border-atlas-line">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <p className="text-sm font-semibold text-atlas-ink">{title}</p>
                  <span className="text-xs text-atlas-ink-faint italic">{who}</span>
                </div>
                <p className="text-sm text-atlas-ink-muted">{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Section title="8. Download Forecast — Excel Output">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-atlas-surface border border-atlas-line">
            <p className="text-sm font-semibold text-atlas-ink mb-2">Channels Sheet</p>
            <ul className="text-xs text-atlas-ink-muted space-y-1.5">
              <li><span className="text-atlas-ink-faint mr-1">—</span>Rows: One per active SKU</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Columns: SKU metadata (New Master SKU, FG Code, Product Name…) then one column per Channel per Month (M1, M2, M3)</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Grand Total column per month (SUM formula)</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Subtotal row (SUBTOTAL formula — filter-aware)</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Missing forecasts shown as 0</li>
            </ul>
          </div>
          <div className="p-4 rounded-xl bg-atlas-surface border border-atlas-line">
            <p className="text-sm font-semibold text-atlas-ink mb-2">Consolidated Sheet</p>
            <ul className="text-xs text-atlas-ink-muted space-y-1.5">
              <li><span className="text-atlas-ink-faint mr-1">—</span>Rows: One per active SKU</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Columns: SKU metadata then one column per Cluster per Month</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Values pulled via SUMIFS cross-sheet formulas from the Channels sheet</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>Subtotal row (SUBTOTAL formula)</li>
              <li><span className="text-atlas-ink-faint mr-1">—</span>File named: Forecast_MonthYear_V{"{version}"}.xlsx</li>
            </ul>
          </div>
        </div>
      </Section>
      ...
      <Section title="9. Carry-Forward">
        <p className="text-sm text-atlas-ink-muted mb-4">
          When creating a new version of the same forecast month, use <strong className="text-atlas-ink">Carry-Forward</strong> to seed it with the previous published version's data — so KAMs only need to revise what changed.
        </p>
        <Steps steps={[
      ...
      <Section title="10. Audit Log">
        <p className="text-sm text-atlas-ink-muted">
          ...
          Actions include: <span className="text-atlas-ink">update_user_role · carry_forward · delete_cycle · bulk_upload · discontinue_sku · reactivate_sku</span> and more.
        </p>
        <InfoBox>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="text-base font-semibold text-atlas-ink mb-4 pb-2 border-b border-atlas-line">{title}</h3>
      {children}
    </div>
  );
}
...
function Steps({ steps }: { steps: string[] }) {
  return (
    <ol className="space-y-2">
      {steps.map((s, i) => (
        <li key={i} className="flex gap-3 text-sm text-atlas-ink-muted">
          <span className="flex-shrink-0 w-5 h-5 rounded-full bg-atlas-surface-soft text-atlas-ink-faint text-xs font-medium flex items-center justify-center mt-0.5">
            {i + 1}
          </span>
          {s}
        </li>
...
function RoleRow({ badge, color, scope, caps }: { badge: string; color: string; scope: string; caps: string }) {
  const colors: Record<string, string> = {
    ...
    gray: "bg-atlas-surface-soft text-atlas-ink",
  };
  return (
    <tr>
      ...
      <td className="py-2.5 pr-6 text-sm text-atlas-ink-muted whitespace-nowrap">{scope}</td>
      <td className="py-2.5 text-sm text-atlas-ink-muted">{caps}</td>
    </tr>
  );
}
