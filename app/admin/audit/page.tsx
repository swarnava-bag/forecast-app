      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        {entries.length === 0 ? (
          ...
          <div className="divide-y divide-gray-800/50">
            {entries.map((entry) => (
              <div key={entry.id} className="px-6 py-4 hover:bg-gray-800/20 transition">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${actionColors[entry.action] || "bg-gray-700 text-gray-300"}`}>
                      {formatAction(entry.action)}
                    </span>
                    <span className="text-sm text-white">{entry.user_email}</span>
                  </div>
                ...
                {entry.new_values && (
                  <details className="mt-1">
                    ...
                    <pre className="mt-1 text-xs text-gray-500 bg-gray-800/50 p-2 rounded overflow-x-auto">
